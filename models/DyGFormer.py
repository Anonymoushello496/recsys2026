...
from models.modules import TimeEncoder,NoTimeEncoder,CosineTimeEncoder,SineCosineTimeEncoder,LinearTimeEncoder
...
class DyGFormer(nn.Module):
    ...
    def __init__(
        ...
        elif time_encoding_method == "original":
        ...
        elif time_encoding_method == "linear2":
            self.time_encoder = LinearTimeEncoder(time_dim=time_feat_dim,mean=avg_time_diff, std=std_time_diff)
...
#ok
    def get_features(self, node_interact_times: np.ndarray, padded_nodes_neighbor_ids: np.ndarray, padded_nodes_edge_ids: np.ndarray,
                     padded_nodes_neighbor_times: np.ndarray, time_encoder: NoTimeEncoder):
        """
...
#changes in the attn score in each fn  , check for it

#class dygformer module
#transformer encoder 
