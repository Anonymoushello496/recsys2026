...
def convert_to_gpu(*data, device: str = "cuda", use_data_parallel: bool = False):
    """
    Move data or models to GPU. Supports multi-GPU if use_data_parallel=True.
    :param data: any objects (Tensor, Module, etc.)
    :param device: device string, e.g. 'cuda' or 'cuda:0'
    :param use_data_parallel: whether to wrap model(s) in DataParallel for multi-GPU use
    """
    res = []
    ...
    for item in data:
        if isinstance(item, nn.Module):
            # Always move to cuda:0 for DataParallel
            base_device = "cuda:0" if (use_data_parallel and n_gpus > 1) else device
            item = item.to(base_device)

            if use_data_parallel and n_gpus > 1:
                print(f"🔹 Using {n_gpus} GPUs via DataParallel")
                item = nn.DataParallel(item)
