...
def get_link_prediction_args(is_evaluation: bool = False):
    ...
    parser.add_argument('--timeenc', type=str, help='linear or original', default='original',
                        choices=['original', 'linear','linear2'])
    parser.add_argument('--path', type=str, help='data path', default='hello')
    parser.add_argument('--edgepath', type=str, help='edge data path', default='hello')
    parser.add_argument('--nodepath', type=str, help='node data path', default='hello')
    ...
    parser.add_argument('--splitbatch', type=int, default=1024, help=' split batch size')
    parser.add_argument('--scale', type=int, default=0, help='time scaling')
    parser.add_argument('--num_neighbors2', type=int, default=20, help='temp neighbors')
    ...
    parser.add_argument('--test_interval_epochs', type=int, default=20, help='how many epochs to perform testing once')
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            if n_gpus > 1:
                args.device = 'cuda'  # <-- allow DataParallel to scatter
                print(f"🔹 Using {n_gpus} GPUs (DataParallel mode)")
            else:
                args.device = f'cuda:{args.gpu}' if args.gpu >= 0 else 'cuda:0'
        else:
            args.device = 'cpu'
            print("⚠️ Using CPU")
    except:
