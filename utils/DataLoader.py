...
def get_link_prediction_data(dataset_name: str, val_ratio: float, test_ratio: float,args):
    ...
    location=args.path
    graph_df = pd.read_csv(location.format(dataset_name, dataset_name))

    # edgepath=args.edgepath
    # nodepath=args.nodepath
    # edge_raw_features = np.load(edgepath.format(dataset_name, dataset_name))
    # node_raw_features = np.load(nodepath.format(dataset_name, dataset_name))
    # edge_raw_features = np.load(edgepath.format(dataset_name, dataset_name),allow_pickle=True)
    # node_raw_features = np.load(nodepath.format(dataset_name, dataset_name),allow_pickle=True)
    # edge_raw_features = np.load('/kaggle/working/rel2.npy'.format(dataset_name, dataset_name),allow_pickle=True)
    # node_raw_features = np.load('/kaggle/working/node2.npy'.format(dataset_name, dataset_name),allow_pickle=True)
