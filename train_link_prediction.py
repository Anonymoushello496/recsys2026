...
from utils.analysis import analyze_target_historical_event_time_diff

class FullModel(nn.Module):
    def __init__(self, dynamic_backbone, link_predictor):
        super().__init__()
        self.dynamic_backbone = dynamic_backbone
        self.link_predictor = link_predictor

    def forward(self, *args, **kwargs):
        raise NotImplementedError("Call submodules explicitly: .dynamic_backbone() or .link_predictor()")
...
if __name__ == "__main__":
    ...
    scale=args.scale # can be changed lets see
    ##time diff stats calculation
    # if scale:
    #         # get the time difference statistics in the training set
    #         node_ids = np.concatenate([train_data.src_node_ids, train_data.dst_node_ids])
    #         node_interact_times = np.concatenate(
    #             [train_data.node_interact_times, train_data.node_interact_times]
    #         )
    #         (
    #             _,
    #             _,
    #             nodes_neighbor_times_list,
    #         ) = train_neighbor_sampler.get_all_first_hop_neighbors(
    ...
    #             512,
    #         )
    #         avg_time_diff = np.nanmean(avg_time_diffs_per_tgt_edge)
    #         std_time_diff = np.nanstd(avg_time_diffs_per_tgt_edge)
    #         median_time_diff = np.nanmean(median_time_diffs_per_tgt_edge)
    # else:
    #     avg_time_diff = 0
    #     median_time_diff = None
    #     std_time_diff = 1
    
    import cupy as cp

# ✅ Explicitly choose one GPU (0 = first GPU)
    cp.cuda.Device(0).use()

    if scale:
        src_ids_gpu = cp.asarray(train_data.src_node_ids)
        dst_ids_gpu = cp.asarray(train_data.dst_node_ids)
        interact_times_gpu = cp.asarray(train_data.node_interact_times)
        
        
        print("scaling")

        # Concatenate on GPU 0
        node_ids_gpu = cp.concatenate([src_ids_gpu, dst_ids_gpu])
        node_interact_times_gpu = cp.concatenate([interact_times_gpu, interact_times_gpu])
        
        print("scaling2")

        # Move minimal data back to CPU for the sampler
        node_ids = cp.asnumpy(node_ids_gpu)
        node_interact_times = cp.asnumpy(node_interact_times_gpu)
        
        print("scaling3")

        # Sampler stays on CPU
        _, _, nodes_neighbor_times_list = train_neighbor_sampler.get_all_first_hop_neighbors(
            node_ids=node_ids, node_interact_times=node_interact_times
        )
        print("scaling4")
        try:
    # Delete large GPU arrays if no longer needed
            del src_ids_gpu, dst_ids_gpu, interact_times_gpu
            del node_ids_gpu, node_interact_times_gpu
        except NameError:
            pass  # safe if some don't exist

        # Clear CuPy cache
        cp._default_memory_pool.free_all_blocks()
        cp.get_default_pinned_memory_pool().free_all_blocks()

        # Clear unused Python objects
        gc.collect()

        print("🔹 Memory cleared before analysis")
        # Back to GPU for fast statistics
        (
            avg_time_diffs_per_tgt_edge,
            median_time_diffs_per_tgt_edge,
            _,
            ...
            nodes_neighbor_times_list, node_interact_times, args.num_neighbors2
        )
        print("scaling5")
        avg_gpu = cp.asarray(avg_time_diffs_per_tgt_edge)
        med_gpu = cp.asarray(median_time_diffs_per_tgt_edge)

        avg_time_diff = float(cp.nanmean(avg_gpu).get())
        std_time_diff = float(cp.nanstd(avg_gpu).get())
        median_time_diff = float(cp.nanmean(med_gpu).get())
    ...
    for run in range(args.num_runs):
        ...
        if args.timeenc=='original':
            timedim=100
        elif args.timeenc=='linear':
            timedim=1
        elif args.timeenc=='linear2':
            timedim=2
        ...
        if args.model_name == 'TGAT':
            ...
            dynamic_backbone = DyGFormer(node_raw_features=node_raw_features, edge_raw_features=edge_raw_features, neighbor_sampler=train_neighbor_sampler,
                                         time_feat_dim=timedim, channel_embedding_dim=args.channel_embedding_dim,output_dim=node_raw_features.shape[1], patch_size=args.patch_size,
                                         num_layers=args.num_layers, num_heads=args.num_heads, dropout=args.dropout,
                                         max_input_sequence_length=args.max_input_sequence_length,embed_method='original',time_encoding_method=args.timeenc,avg_time_diff =avg_time_diff,std_time_diff =std_time_diff,time_channel_embedding_dim=None,use_positional_embedding=False, device=args.device)
        # AKedits
        else:
        ...
        # dynamic_backbone = convert_to_gpu(dynamic_backbone, device=args.device, use_data_parallel=(args.gpu > 1))
        # link_predictor = convert_to_gpu(link_predictor, device=args.device, use_data_parallel=(args.gpu > 1))

        # model = FullModel(dynamic_backbone, link_predictor)
        ...
        model = convert_to_gpu(model, device=args.device,use_data_parallel=(args.gpu>1))
        ...
        base_folder = "/kaggle/working" if os.path.exists("/kaggle/working") else "."
        ...
        # model0=unwrap_model(model.module[0])
        # model1=unwrap_model(model.module[1])
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                if args.model_name in ['TGAT', 'CAWN', 'TCL']:
                    ...
                    batch_src_node_embeddings, batch_dst_node_embeddings = \
                        model0.compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
                    ...
                    batch_neg_src_node_embeddings, batch_neg_dst_node_embeddings = \
                        model0.compute_src_dst_node_temporal_embeddings(src_node_ids=batch_neg_src_node_ids,
